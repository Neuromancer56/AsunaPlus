...
function goblins.update_hud(player)
    if ght > 0 and player then
        ...
        local known_territories = minetest.deserialize(meta:get_string(
                                                           "territory_list"))
        local territory = "unknown"
        if known_territories then
            for k, v in pairs(known_territories) do
                if territory_current == v then territory = v end
            end
        end
        local text_goblin_current = goblin_current
        local text_territory_current = "of " .. territory
        local territory_score_table = minetest.deserialize(meta:get_string(
                                                               territory_current))
        ...
        if not ids then
          ...
          --]]
            ...
            params = {player_name, ids.territory_current}
            minetest.after((ght + 5), clear_hud, params)
            local tst_yo = 0
            for k, v in pairs(territory_score_table) do
                tst_yo = tst_yo + 20
                ...
                params = {player_name, ids.territory_scores[k]}
                minetest.after(ght + 5, clear_hud, params)
            end
            ...
            if meta:get_int("hud_cleared") == 1 then
                ...
                local params = {player_name, ids.goblin_current}
                player:hud_change(ids["goblin_current"], "text",
                                  text_goblin_current)
                minetest.after(ght, clear_hud, params)
                params = {player_name, ids.territory_current}
                player:hud_change(ids["territory_current"], "text",
                                  text_territory_current)
                minetest.after(ght, clear_hud, params)
                for k, v in pairs(territory_score_table) do
                    params = {player_name, ids.territory_scores[k]}
                    minetest.after(ght, clear_hud, params)
                    player:hud_change(ids.territory_scores[k], "text",
                                      text_territory_scores[k])
                end
